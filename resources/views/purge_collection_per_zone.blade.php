...
@section('content')
    ...
    @include('statamic-cloudflare-addon::partials.cloudflare-navigation', ['activeTab' => 'purgeCollectionsPerZone'])
    @php
    ...
    @if ($isConfigured)
    <publish-form
            title="Purge Collections Per Zone"
            :blueprint='@json($blueprint["blueprint"])'
            :meta='@json($blueprint["meta"])'
            :values='@json($blueprint["values"])'
            action="{{ cp_route('utilities.cloudflare.purgeCollectionsPerZone') }}"
    ></publish-form>
    @else
        <div class="py-6">
